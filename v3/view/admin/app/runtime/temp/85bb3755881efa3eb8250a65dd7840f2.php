<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:85:"D:\project\v3\view\admin\app\public/../application//merchant/view/admin/page_add.html";i:1556524326;s:82:"D:\project\v3\view\admin\app\public/../application//common/view/common/layout.html";i:1551405380;s:45:"../application/common/view/common/header.html";i:1551405380;s:45:"../application/common/view/common/footer.html";i:1546909578;}*/ ?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
                    ...
                    <div class="layui-tab-content" >
                        <div class="layui-tab-item layui-show">
                            ...
                            <div class="layui-form-item">
                                ...
                                <div class="layui-input-block">
                                    <select name="area_id" lay-verify="required" lay-filter="selectRole">
                                    </select>
                                </div>
                            </div>
                            <div class="layui-form-item">
                                ...
                                <div class="layui-input-block">
                                    <select name="role_id" lay-verify="required" lay-filter="selectRole">
                                    </select>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        layui.use(['jquery', 'form', 'layer', 'laytpl'], function(){
            ...
            request.setHost(CENTER_DATA).get('/merchant/admin/getAreaData',{}, function(res){
                if( res.code == 0 ){
                    var selectDom = $('select[name=area_id]');
                    selectDom.append('<option value="">请选择</option>');
                    var list = res;
                    console.log(list);
                    for(var i in list){
                        selectDom.append('<option data-type="'+list[i].area_id+'" value="'+list[i].id+'">'+list[i].title+'</option>');
                    }
                    form.render();
                }else{
                    layer.msg(res.msg);
            ...
            request.setHost(CENTER_DATA).get('/merchant/admin/getAddData',{}, function(res){
                if( res.code == 0 ){
                    var selectDom = $('select[name=role_id]');
                    selectDom.append('<option value="">请选择</option>');
                    var list = res.data.roleList;
                    for(var i in list){
                        selectDom.append('<option data-merchant_id="'+list[i].merchant_id+'" data-type="'+list[i].type+'" value="'+list[i].id+'">'+list[i].title+'</option>');
                    }
                    form.render();
                }else{
                    layer.msg(res.msg);
            ...
            form.on('submit(submitBtn)', function(data){
                request.setHost(CENTER_DATA).post('/merchant/admin/add', data.field, function(res){
                    if (res.code == 0) {
                        layer.msg(res.msg);
                        setTimeout(function(){

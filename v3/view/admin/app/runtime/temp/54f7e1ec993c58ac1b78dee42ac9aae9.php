<?php if (!defined('THINK_PATH')) exit(); /*a:4:{s:89:"D:\project\v3\view\admin\app\public/../application//merchant/view/meregion/page_list.html";i:1556610244;s:82:"D:\project\v3\view\admin\app\public/../application//common/view/common/layout.html";i:1551405380;s:45:"../application/common/view/common/header.html";i:1551405380;s:45:"../application/common/view/common/footer.html";i:1546909578;}*/ ?>
<!doctype html>
<html>
<head>
<meta charset="utf-8">
...
<div class="layui-fluid">
    <div class="layui-card">
        <div class="layui-card-body">
            <table id="meregion" lay-filter="meregion"></table>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    layui.use(['form','table','layer'], function(){
        ...
        //监听行工具事件
        table.on('tool(meregion)', function (obj) {
            const data = obj.data
            ...
            switch(layEvent){
                case 'edit':
                    layer.open({
                        ...
                        scrollbar:false,
                        shadeClose:true,
                        content: '/merchant/meregion/page_edit?id='+data.id
                    })
                    break;
                case 'del':
                    layer.confirm("是否确认删除！删除后 无法恢复！", {btn: ["确定","取消"] },
                        function(){
                            request.setHost(CENTER_DATA).get('/power/meregion/delete',{ids: data.id}, function(res){
                                if (res.code == 0) {
                                    // 成功提示
                                    layer.msg('删除成功');
                                    setTimeout(function(){
                                        table.reload('meregion');
                                    },1000);
                                } else {
                                    // 错误提示
                                    layer.msg(res.msg);
        ...
        //监听工具栏事件
        table.on('toolbar(meregion)', function (obj) {
            const data = obj.data
            const layEvent = obj.event
            if (layEvent === 'add') {
                layer.open({
                    ...
                    scrollbar:false,
                    shadeClose:true,
                    content: '/merchant/meregion/page_add'
                })
            }
        });

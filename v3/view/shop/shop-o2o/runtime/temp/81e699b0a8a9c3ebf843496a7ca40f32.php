...
<script type="text/javascript">
	...
	function initTable(params){
	  ...
	  table.render({
	    ...
	    ,cols: [[ //表头
	      ...
	      ,{field: 'receipt_time', title: '收货时间', width:180}
	...
	function tableDoneCallBack(){
		...
		$('#cancel-btn').on('click', function(){
			...
			if (selectRow) {
				...
				layer.confirm('确认要作废吗?作废后不能恢复的', function(index){
				  ...
				  // 提交数据
				  request.post('/depot/shop_purchase_order/cancel', {id: selectRow.id}, function(res){
				  	// 判断返回结果
				  	if (res.code == 0) {
	    ...
	    $('.edit-order').on('click', function(){
			...
			// 打开进货单商品列表
			location.href = '/depot/purchase/item_list?purchase_id='+id;
	    })

	    // 修改订单
	    $('#edit-btn').on('click', function(){
	    	// 打开进货单商品列表
			location.href = '/depot/purchase/detail?id='+selectRow.id;
	    })

	    // 申请差异单
	    $('#differ-btn').on('click', function(){
	    	// 打开进货单商品列表
			location.href = '/depot/purchase/item_list?id='+selectRow.id+'&is_differ=1';
	    })

...
class Service extends Controller{
    ...
    public function _initialize(){
        ...
        if(session('admin')){
            ...
            //管理员是否超管
            $this->fiexdParams['admin_is_super'] = session('admin.is_super');

            if(empty(session('admin.is_super'))){//不是超管
                //管理员账号类型
                $this->fiexdParams['admin_role_type'] = session('admin.role')['type'];
                //管理员角色id
                $this->fiexdParams['admin_role_id'] = session('admin.role')['id'];
                //管理员商户id
                $this->fiexdParams['admin_merchant_id'] = session('admin.merchant_id');
            }else{
                //管理员账号类型
                $this->fiexdParams['admin_role_type'] = 0;
                $this->fiexdParams['admin_role_id'] = 0;
                $this->fiexdParams['admin_merchant_id'] = 0;
            }

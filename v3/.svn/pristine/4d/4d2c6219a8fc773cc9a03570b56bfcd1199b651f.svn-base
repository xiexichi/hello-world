...
if ( ! function_exists('sizeKeySort')){
    function sizeKeySort($arr, $size_name){
        /**
         * 
         */
 ...
 * @param  [admix]  $default [默认值]
 */
if( !function_exists('execBeforeMethods') )
{
    function execBeforeMethods(&$obj, $query, $methodName) {

        // pe($query);

        $method = $methodName . 'Before';

        if (objHasMethod($obj, $method)) {

            $query = $obj->$method($query);
        }
...
// execAfterMethods
/**
 * [execAfterMethods 执行对象方法+After，名称方法]
 * @param  [string] $name [设置名称]
 * @param  [admix]  $default [默认值]
 */
if( !function_exists('execAfterMethods') )
{
    function execAfterMethods(&$obj, $query, $methodName) {

        // pe($query);

        $method = $methodName . 'After';
...
if(!function_exists('makeRandStr'))
{
	function makeRandStr($length = 6){
		$codeSet = '2345678abcdefhjkmnpqrtwxyz';
		$code = '';
		for ($i = 0; $i < $length; $i++) {

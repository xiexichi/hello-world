...
class Merchant extends Base
{
	...
	/**
	 * 通过merchant_id获取商户列表，如果merchant_id为0则获取全部
	 * @param number $merchantID
	 * @return unknown
	 */
	public function getMerchantListByMerchantID($merchantID = 0){
	    if(!empty($merchantID)){
	        $this->where('id',$merchantID);
	    }
	    return $this->field('id,merchant_type_id,name,desc,is_default')->select();
	}

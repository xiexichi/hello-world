...
class Stock extends Base
{
	 ...
	 */
	public function getItems($item_code, $product_color_id, $shop_id = NULL, $key_type = NULL, $shop_depot_pre_select_id = NULL, $shop_depot_type = NULL){
		...
		$this->alias('a')
			 ->field('a.*,a.id stock_id,IFNULL(sd.availavle_quantity, 0) shop_quantity,"" pre_select_quantity,gi.id goods_item_id')
			 ->join("shop_depot sd", "a.id = sd.stock_id", 'LEFT')
             ->join('goods_item gi', 'CONCAT(a.item_code,",",a.sku_code) = gi.erp_code', 'LEFT') // 查找goods_item表的id
        ...
        if ($shop_depot_type) {
            // 设置店铺库存类型
            $this->where('sd.type', $shop_depot_type);
        }

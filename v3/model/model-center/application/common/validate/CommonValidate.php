...
class CommonValidate extends Validate{
	...
	protected function validate($rule , $data, $msg = NULL, $callback = NULL){
		// 创建验证器
		if ($msg) {
			$validate = new CommonValidate($rule, $msg);
		} else {
			$validate = new CommonValidate($rule);
		}
	...
	public function notOnlyInt($value,$rule,$data){
		if((int)$value == $value){
			return false;
		}
		return true;
	...
	public function mustPhone($value, $rule, $data){
		$pregPhone='/^1[3456789]\d{9}$/ims';
		if(preg_match($pregPhone,$value)){
			return true;
		}
		return false;
	}

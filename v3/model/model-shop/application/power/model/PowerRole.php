...
class PowerRole extends Base
{
	 ...
	 */
	public function getRoleGroupTree($selectedRoleID = 0){
	    $moduleList = (new PowerModule())->field('id,title')->order('sort DESC')->select();
	    $controllerList = (new PowerController())->field('id,title,module_id')->order('sort DESC')->select();
	    $groupList = (new PowerGroup())->field('id,title,module_id,controller_id')->order('sort DESC')->select();

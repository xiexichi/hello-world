...
class ShopDepot extends Base
{
	...
	public function getStockInfo(){
		...
		$data = $this->alias('a')
					 ->field('a.*,a.availavle_quantity shop_quantity,gi.item_img,gi.item_price,s.salable_qty,s.item_code,s.sku_code')
					 ->join('stock s', 'a.stock_id = s.id')
					 ->join('goods_item gi', 'CONCAT(s.item_code,",",s.sku_code) = gi.erp_code', 'LEFT')
				 	 ->where('a.type', $this->data['type'])	// 一定是商品
				 	 ->where('a.stock_id', $this->data['stock_id'])	// 商品库存id
				 	 ->where('a.shop_id', $this->data['shop_id'])	// 店铺id
				 	 ->find();
	...
	public function indexAfter(){
		...
		foreach ($this->data as $k => $v) {
			...
			// 如果有店铺库存类型
			if (!empty($this->tempData['type'])) {
				$stocks = $stockModel->getItems($v['item_code'], $v['product_color_id'], $shopId, 'size', $shopDepotPreSelectId, $this->tempData['type']);
			} else {
									   ...
									   $type = self::TYPE_SALES)
	{
		...
		// ★★☆预扣返还类型
		$reverseArr = ShopDepotChange::getReverse();
		 ...
		 * 
		 */
		if ( in_array($operation_type, $reverseArr) ) {

			// 获取最大返回数量
			$MaxReverseQuantity = abs($this->getMaxReverse($order_sn, $stock_id, $quantity));
			 ...
			 * 2.并且返回库存大于已预扣库存
			 */
			if ($MaxReverseQuantity > 0 && $quantity > $MaxReverseQuantity) {
				// 只能返还最大数量
				$quantity = $MaxReverseQuantity;
			}
		}
		...
		if ($item) {
			...
			// ★★☆预扣与预扣返还类型
			$withholdArr = ShopDepotChange::getWithAndReverse();
	...
	/**
	 * [getMaxReverse 获取最大返还数量]
	 * @param  [type] $order_sn [订单号]
	 * @param  [type] $stock_id [库存id]
	 * @param  [type] $quantity [数量]
	 * @return [type]           [description]
	 */
	protected function getMaxReverse($order_sn, $stock_id, $quantity){
		// pe($order_sn);
		// 库存变动模型
		$sdcModel = new ShopDepotChange();
			
		// 查找已处理数量
		$handleQuantity = $sdcModel->alias('a')
							 ->join('shop_depot sd', 'a.shop_depot_id = sd.id')
							 ->where('order_sn', $order_sn)
							 ->where('sd.stock_id', $stock_id)
							 ->sum('a.quantity');

		// pe($sdcModel->getLastSql());
	...
	public function batchDepotOperating($order_sn, $operation_type, $shop_id, $stock_id, $quantity){
		...
		if ($quantity > 0) {
			...
			if ($operation_type == ShopDepotChange::TYPE_SALES_RETURN
				...
				// 如果是进货差异类型，则添加查找对应单号的进货批次
				if ($operation_type == ShopDepotChange::TYPE_SHOP_DIFFER) {
					// 添加指定进货单号
					$spiModel->where('spo.order_sn', $order_sn);
				}
	...
	protected function batchDepotOperatingReduce($order_sn, $operation_type, $shop_id, $stock_id, $quantity, $sort){
		...
		while ( $whileQuantity > 0 ) {
			...
			$item = $spiModel->alias('a')
							 ...
							 ->find();
			// 库存信息
			$stockInfo = $stockModel->getStockField($stock_id, 'item_code') .' '. $stockModel->getStockField($stock_id, 'sku_code');
    ...
    /**
     * 获取商品库存
     * @param $goods_erp_code   商品erp编码
     * @param int $shop_id      店铺id
     */
	public function getShopGoodsDepot($goods_erp_code,$shop_id=1){
	    $where = [
	        's.item_code'=>$goods_erp_code,
            'sd.type'=>1,
        ];
	    if( $shop_id > 0 ){
        ...
        $stock_list = $this->alias('sd')
            ->field('sd.shop_id,sd.availavle_quantity,s.item_code,s.sku_code,s.sku_name,gi.id as item_id')
            ->join($stockModel->getTable().' s','s.id = sd.stock_id')
            ->join($goodsItemModel->getTable().' gi','gi.erp_code = CONCAT(s.item_code, ",", s.sku_code)')
            ->where($where)
            ->select();
        return $stock_list;
	}

...
//                    $disVal = ['1'=>[1,4],'2'=>[1,2]];//测试用 参数不用的话看这里就行了
                    if( !is_array($disVal) ){
                        $this->error = '优惠券使用错误';
                        return false;
                    }
    ...
    public function getBuyGoodsList($user_id,$shop_id=0){
        ...
        if( $activity > 0 ){
            //获取活动内容
            $this->discount_content['activity'][$shop['shop_id']][] = '买满XXX然后XXX的优惠';
        }
        //余额抵扣
        //获取合计信息 不含运费
        ...
        foreach( $cart_list as $skey => $shop ){
            ...
            foreach( $shop['goods_list'] as $gkey => $goods  ){
                ...
                if( $goods['gift_id'] == 0 ){
                    ...
                    $this->discount_content['activity'][$shop['shop_id']][] = $goods['activity_content'].',赠品：'.$goods['item_info']['erp_code'];
                }

...
class GoodsItem extends Base
{
    ...
    public function createGoodsSku($goods_id,$goodsInfo){
        $t = date('Y-m-d H:i:s');
        //默认创建一个sku 若添加限制必须设置这里就没必要存在了
        // 因下单是以sku为单位
        // 所以创建商品时 设置sku 项目 非必设 情况下要默认创建一个没属性sku
//        $item_insertData = [
//            'goods_id'          => $goods_id,
...
//        }
    ...
    public function getGoodsItems($goods_id,$where=[]){
        ...
        $subQuery = $goodsItemPropValModel->alias('ipv')
            ->field('group_concat(pv.id ORDER BY p.sort DESC) as pv_id')
...
//        $list = $this->query($sql);
    ...
    public function getItemInfo($item_id){
        ...
        $info = $this->alias('i')
            ->field([
                'g.goods_name',
                'g.erp_code as default_erp_code',
                'g.ship_free',
                'g.weight',
                'g.verify as goods_verify',
                'g.sales_status as goods_sales',
                'g.is_deleted as goods_deleted',
                'i.*',
                'if(i.item_img = "",img.image,i.item_img) as item_img',
                $subQuery.' as pv_name'
            ])

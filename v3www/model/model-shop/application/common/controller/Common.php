...
class Common extends Controller
{
	...
	// 25boy总后台访问的key
	const REQUEST_ADMIN_KEY = '25boy-admin-livermore';
  ...
  //   	}
    ...
    public function oneBefore(){
    	...
    	// 如果有指定的查参数
    	if (objHasPropertie($this, 'oneParams')) {
    		if (is_array($this->oneParams)) {

    			foreach ($this->oneParams as $k => $v) {
    				if (input($v)) {
    					$params[$v] = input($v);
    				}
    			}
    			...
    			$params[$this->oneParams] = input($this->oneParams);
    		}
    	} else {
    		...
    		// 输出错误提示
	    	if ($id) {
	    		$a = $this->model->getAlias();
	    		if ($a) {
	    			$this->model->where($a.'.id', $id);
	    		} else {
    ...
    // 		$this->model->data($params);
	   ...
	   //  	// 查找数据
	   //  	// $this->model->data($params)->where($this->model->getTable().'.'.$this->model->getPkName(), input('param.id'));
    // 	}
    // }

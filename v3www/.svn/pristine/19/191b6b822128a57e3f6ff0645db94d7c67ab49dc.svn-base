...
class Goods extends Base
{
    ...
    public function indexBefore(){
        ...
        if( !empty($param['min_price']) && empty($param['max_price']) ) {
            $where['g.sell_price'] = ['>=', $param['min_price']];
        }else if( empty($param['min_price']) && !empty($param['max_price']) ){
            $where['g.sell_price'] = ['<=', $param['max_price']];
        }else if( !empty($param['min_price']) && !empty($param['max_price']) ){
            $where['g.sell_price'] = ['between', [$param['min_price'],$param['max_price']]];
        }
    ...
    public function getGoodsInfo(){
        ...
        $ids = $goodsInfo['id'];
        $shop_id = isset($goodsInfo['shop_id']) ? $goodsInfo['shop_id'] : 0;//这里默认无店铺
        $returnCode = $ids;
        try{
            if( empty($ids) ){
                $returnCode = 020101;
            ...
            $goods_stock = $shopDepotModel->getShopGoodsDepot($info['erp_code'],$shop_id);
            $sku_stock = [];
            foreach( $goods_stock as $stock ){
                $sku_stock[$stock['item_id']] = $stock;
            }
            if(!empty($sku_all)){
                foreach( $sku_all as $sku){
                    ...
                    $stock = 0;
                    if( isset($sku_stock[$sku['id']]) ){
                        $stock = $sku_stock[$sku['id']]['availavle_quantity'];
                    }
    ...
    public function getInfo(){
        ...
        $ids = $param['id'];
        $shop_id = isset($param['shop_id']) ? $param['shop_id'] : 1;//默认写死1
        ...
        try{
            ...
            $shopDepotModel = new \app\depot\model\ShopDepot();
            $goods_stock = $shopDepotModel->getShopGoodsDepot($goodsInfo['erp_code'],$shop_id);
            $sku_stock = [];
            foreach( $goods_stock as $stock ){
                $sku_stock[$stock['item_id']] = $stock['availavle_quantity'];
            }
            //附加item信息
            $info['item_info']['stock'] = isset($sku_stock[$goodsInfo['item_id']]) ? $sku_stock[$goodsInfo['item_id']] : 0;
            $item_list = [];
            ...
            foreach( $prop_all as $pro_key => $prop_id ) {
                ...
                foreach( $info['prop_val_list'][$prop_id] as $pv_id => $pv_name ){
                    ...
                    $list_info['stock'] = isset($sku_stock[$list_info['item_id']]) ? $sku_stock[$list_info['item_id']] : 0;
    ...
    public function resetItemInfo(){
        ...
        $shopDepotModel = new \app\depot\model\ShopDepot();
        $goods_stock = $shopDepotModel->getShopGoodsDepot($itemInfo['default_erp_code'],$shop_id);
        $sku_stock = [];
        foreach( $goods_stock as $stock ){
            $sku_stock[$stock['item_id']] = $stock['availavle_quantity'];
        }
        //附加item信息
        $info['item_info']['stock'] = isset($sku_stock[$item_id]) ? $sku_stock[$item_id] : 0;
        $item_list = [];
        foreach( $item_all as $key => $item ){
            $item_list[$item['pv_id']] = $item['id'];
        }
    ...
    //获取单个商品的运费信息
    public function getGoodsShipPrice(){
        if( !$param = $this->validate->getGoodsShipPrice(input()) ){
            return errorJson(000001, $this->validate->getError());
        }
        //获取商品信息
        $orderModel = new \app\order\model\Order();
        $shop_id = empty($param['shop_id']) ? 1 : $param['shop_id'];
        $ship_fee = $orderModel->getItemShip($shop_id,$param['item_id'],$param['num'],$param['delivery_id'],$param['prov_id']);
        return successJson($ship_fee,'SUCCESS');
    }

    ...
    if(!empty($last_id)){
        ...
        if ($notify->data["return_code"] == "FAIL") {
            ...
            */
            ...
            // 逻辑处理
            if(!empty($bag) && $bag['pay_status']=='payment'){
                // 查询用户余额
                ...
                */
                ...
                if(strpos($bag['note'],'合并付款') === false){
                    ...
                    // 不可与充值活动共用
                    if(empty($plus_price)){
                        $firstRecharge = $Common->firstRecharge($bag['user_id'], $total_fee);
                        if(!empty($firstRecharge['plus_price'])){
                            $plus_price = $firstRecharge['plus_price'];
                            $bagSet['note'] = $firstRecharge['title'];
                            $bagSet['plus_price'] = $plus_price;
                        }
                    }
                }

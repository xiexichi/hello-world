...
var configarray = {
    ...
    "desc":wxconfigarray.desc?wxconfigarray.desc:'225BOY国潮男装'
};
wx.config({
    debug: false, appId: '{$signPackage.appId}', timestamp:"{$signPackage.timestamp}", nonceStr: '{$signPackage.nonceStr}', signature: '{$signPackage.signature}', jsApiList: ['onMenuShareTimeline', 'onMenuShareAppMessage', "onMenuShareQQ", "onMenuShareWeibo", "onMenuShareQZone","chooseImage","previewImage","uploadImage"]
});
wx.ready(function () {
    /*获取“分享到朋友圈”按钮点击状态及自定义分享内容接口*/
    wx.onMenuShareTimeline({
        title: configarray['title']+(configarray['desc']?' | '+configarray['desc']:''),
        link: configarray['link'],
        imgUrl: configarray['imgUrl'],
        success: function () {
            $.getJSON('/ajax/share.callback.php?type=lottery',function(res){
                if(res['ms_code']=='success'){
                    var notice_btn=[];
                    if(res['ms_code']=='success'){
                        notice_btn = [{
                            'title':'我的抽奖码',
                            'url':'/?m=account&a=coupon&c=lottery'
                        }]
                    }
                    shownotice({
                        "icon":"success",

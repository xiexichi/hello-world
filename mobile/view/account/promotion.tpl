...
<div id="bodybox">
    <section class="main pagemain pagegrey" style="background-color: #efefef">
        ...
        {else}
            ...
            <section class="nav-tags nav-tags-col2 balance_class borderbottom ">
                <span class="item {if $c=='pp'}current{/if}"><a href="/?m=account&a=promotion&c=pp">购物</a></span>
                <span class="item {if $c=='re'}current{/if}"><a href="/?m=account&a=promotion&c=re">充值</a></span>
            </section>

            <section class="order_class borderbottom" show="0">
                <span class="arrow"></span>
                <ul>
                    <li rel="0"  style="background:#f0f0f0;">选择消费时间段（{$when_cn}）</li>
                    <li rel="today"  {if $when=="today"} class="selected"{/if}>今日</li>
                    <li rel="yesterday"  {if $when=="yesterday"} class="selected"{/if}>昨日</li>
                    <li rel="this_week"  {if $when=="this_week"} class="selected"{/if}>本周</li>
                    <li rel="last_week"  {if $when=="last_week"} class="selected"{/if}>上周</li>
                    <li rel="this_month" {if $when=="this_month"}class="selected"{/if}>本月</li>
                    <li rel="last_month" {if $when=="last_month"}class="selected"{/if}>上月</li>
                </ul>
            </section>
                    ...
                    {foreach $promote_product as $k=>$v} 
                      <li>
                            ...
                            <div class="list_right">
                                <dl>
                                   <dt class="fs1_2">{$v['product_name']}</dt> 
                                   <dd class="right_dl_left">
                                        <p class="promotion-click">点击数：<span class="pr1">{$v['click_num']}</span>付款笔数：<span>{$v['paid_order_num']}</span></p>
                                        <p class="promotion-paid-price">效果预估：<span class="fs1_3">¥{$v['paid_order_total']}</span></p> 
                                        <p class="promotion-received-price">预估收入：<span class="fs1_5 C_da3335 bold">¥{$v['received_order_total']}</span></p>
                                   </dd>
                                   <dd class="right_dl_right">
                                        {if $v['is_valid']}
                                        <button data-url="{$v['url']}" data-title="{$v['product_name']}" data-link="{$v['link']}" onclick="promote_now(this,{$promote['promote_id']},0,{$v['product_id']});">获取链接</button>
                                        {else}
                                        <button class="disabled">已失效</button>
                                        {/if}
                                        </dd>
                                </dl>
            ...
            {elseif $c=='re'}
                {if $is_promote_recharge}
                  <section class="product_list">
                    <ul id="listBox">
                      <li>
                            <div class="list_left"><a href="?"><img src="/statics/img/logo_128x128.png" /></a></div>
                            <div class="list_right">
                                <dl>
                                   <dt class="fs1_2">充值返佣</dt> 
                                   <dd class="right_dl_left">
                                        <p class="promote_recharge">充值笔数：<span class="pr1">{$promote_recharge['recharge_num']}</span>平均佣金比率：<span>{$promote_recharge['recharge_average_rate']}%</span></p>
                                        <p class="promotion-paid-price">充值金额：<span class="fs1_3">¥{$promote_recharge['recharge_total']}</span></p> 
                                        <p class="promotion-received-price">预估收入：<span class="fs1_5 C_da3335 bold">¥{$promote_recharge['recharge_earnings_total']}</span></p>
                                   </dd>
                                   <dd class="right_dl_right">
    ...
    <div class="loaddiv" style="display: none;padding-top: 1em;">
...
var when="{$when}";
var height = 285;
var hrefString = "?m=account&a=promotion&c="+c+"&when=";
</script>
<script src="/statics/js/promotion.js?v={$version}"></script>

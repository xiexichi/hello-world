    ...
    for (var i = 0; i < json.length; i++) {
        ...
        if(json[i].stock==0){
            ...
            if(json[i].miao_price > 0){
                html += '<div class="price_box" data-val="'+json[i].miao_price+'"><del><sup>￥</sup>'+json[i].price+'</del> <sup>￥</sup>'+json[i].miao_price+siglequantity+'</div>';
            }else{
                html += '<div class="price_box" data-val="'+json[i].price+'"><sup>￥</sup>'+json[i].price+siglequantity+'</div>';
            }
...
function checkselected(){
    ...
    $(".cartlistbox ul li").each(function(){
        if(!$(this).find(".checkbox").hasClass("checkbox_checked")){
            ...
            if($(this).find(".price_box").length>0){
                total_price += $(this).find("input").val()*Math.floor($(this).find(".price_box").data('val'));
            }
    ...
    $("nav#cartbottom .selectstatus").html("总价:<sup>￥</sup>"+total_price+"<br><span>不含运费</span>");
}

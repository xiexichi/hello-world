...
class image {
    ...
    public function thumb($targetfile,$filename,$_info,$new_w=160,$new_h=120,$cut=0,$big=0){
    // 获取原图信息
        $info  = $_info;
    ...
    public function water($filename,$water,$pos=0,$pct=50){// 加载水印图片
        ...
        $fun = 'image'.$type;
        $type=="png" ? $quality = 1 : $quality = 95;
        !@$fun($oldimg, $filename, $quality) && die('保存失败!检查目录是否存在并且可写?');

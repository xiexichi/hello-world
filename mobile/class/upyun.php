...
class Upyun {
    ...
    public function __construct($endpoint = NULL, $timeout = 30) {/*{{{*/
        ...
        $this->_bucketname = 'miao-photo';
        $this->_username = '25miao';
        $this->_password = md5('mpcPAie03D');
    ...
    protected function _do_request($method, $path, $headers = NULL, $body= NULL, $file_handle= NULL) {/*{{{*/
        ...
        if ($http_code == 0) show_error('Connection Failed', $http_code);
        ...
        if ($http_code == 200) {
            ...
            switch($http_code) {
                case 401:
                    ...
                    //show_error($message);
                    $array = array('http_code'=>$http_code,'message'=>$message);
                ...
                default:
                    ...
                    //show_error($message .':'. $http_code, $http_code);
                    $array = array('http_code'=>$http_code,'message'=>$message);
                    return json_encode($array);
            }
        }
